#!/usr/bin/env python3
import rclpy
from rclpy.node import Node
import time
from my_robot_interfaces.action import CountUntil
from rclpy.action import ActionClient
from rclpy.action.client import ClientGoalHandle, GoalStatus

class CountUntilClientNode(Node):
    def __init__(self):
        super().__init__("count_until_client")
        self.count_until_client = ActionClient(self, CountUntil,"count_until")

    def send_goal(self):
        # Get user input for goal parameters
        print("Enter target number(int) and period(float) to count until:")
        target_number = int(input("Target number: "))
        period = float(input("Period: "))

        # Wait for server to be available
        self.count_until_client.wait_for_server()

        # Create goal
        goal = CountUntil.Goal()
        goal.target_number = target_number
        goal.period = period
        self.count_until_client.send_goal_async(goal, feedback_callback=self.feedback_callback).add_done_callback(self.goal_response_callback)

        # Handle Cancelation
        #self.timer_cancel = self.create_timer(2.0, self.cancel_goal)

    def cancel_goal(self):
        self.get_logger().info("Canceling the goal...")
        self.goal_handle.cancel_goal_async()

    def goal_response_callback(self, future):
        self.goal_handle : ClientGoalHandle = future.result()
        if not self.goal_handle.accepted:
            self.get_logger().info("Goal was rejected by the server.")
            return

        self.get_logger().info("Goal accepted by the server, waiting for result...")
        self.goal_handle.get_result_async().add_done_callback(self.goal_result_callback)

    def goal_result_callback(self, future):
        status = future.result().status
        result = future.result().result
        if status == GoalStatus.STATUS_SUCCEEDED :
            self.get_logger().info("Goal succeeded.")
        elif status == GoalStatus.STATUS_ABORTED:
            self.get_logger().error("Goal was aborted.")
        elif status == GoalStatus.STATUS_CANCELED:
            self.get_logger().warn("Goal was canceled.")

        if not result:
            self.get_logger().info("Result not available.")
            return
        self.get_logger().info(f"Result: " + str(result.reached_number))
        self.send_goal()

    def feedback_callback(self, feedback):
        fed_number = feedback.feedback.current_number
        self.get_logger().info(f"Feedback: Current number is: " + str(fed_number))

def main(args=None):
    rclpy.init(args=args)
    node = CountUntilClientNode()
    node.send_goal()  # Start the goal sending process
    rclpy.spin(node)
    rclpy.shutdown()
     
     
if __name__ == "__main__":
    main()
